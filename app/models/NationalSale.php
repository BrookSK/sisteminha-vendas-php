...
class NationalSale extends Model
{
    ...
    private function correiosTable(float $pesoKg): float
    {
        ...
        $map = [
            1=>35.53,2=>43.27,3=>51.02,4=>58.77,5=>66.52,6=>74.26,7=>82.00,8=>89.74,9=>97.48,10=>105.22,
            11=>112.96,12=>120.70,13=>128.44,14=>136.18,15=>143.92,16=>151.66,17=>159.40,18=>167.14,19=>174.88,20=>182.62,
            21=>190.36,22=>198.10,23=>205.84,24=>213.58,25=>221.32,26=>229.06,27=>236.80,28=>244.54,29=>252.28,30=>260.02
        ];
        if ($pesoKg > 30.0) return 0.0; // não permitido
        $ceil = (int)ceil($pesoKg);
        return $map[$ceil] ?? 0.0;
    }
    ...
    private function compute(array $in): array
    {
        ...
        $frete_brl = $this->correiosTable($peso);
        $frete_usd = $taxa > 0 ? ($frete_brl / $taxa) : 0.0;
        $total_bruto_usd = (float)($in['valor_produto_usd'] ?? 0)
            + (float)($in['taxa_servico_usd'] ?? 0)
        ...
        return [
            'frete_correios_brl' => $frete_brl,
            'frete_correios_usd' => $frete_usd,
            'total_bruto_usd' => $total_bruto_usd,
            'total_bruto_brl' => $total_bruto_brl,
    ...
    private function monthOf(string $date): string { return date('Y-m', strtotime($date)); }
    ...
    private function monthTotals(string $ym): array
    {
        $stmt = $this->db->prepare("SELECT 
            COALESCE(SUM(total_bruto_usd),0) team_bruto,
            COUNT(DISTINCT vendedor_id) seller_count
          FROM vendas_nacionais WHERE DATE_FORMAT(data_lancamento,'%Y-%m') = :ym");
        $stmt->execute([':ym'=>$ym]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
        return [
    ...
    {
        $stmt = $this->db->prepare("SELECT COALESCE(SUM(total_bruto_usd),0) b FROM vendas_nacionais WHERE vendedor_id=:sid AND DATE_FORMAT(data_lancamento,'%Y-%m') = :ym");
        $stmt->execute([':sid'=>$sellerId, ':ym'=>$ym]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
        return (float)($row['b'] ?? 0);
    }

    private function computeCommission(string $ym, int $sellerId, float $liq_usd, float $liq_brl): array
    {
        ...
        $comissao_brl = ($liq_brl * $percent) + $bonus_brl;
        $taxa = $liq_usd > 0 ? ($liq_brl / $liq_usd) : 0.0;
        $comissao_usd = $taxa > 0 ? ($comissao_brl / $taxa) : 0.0;
        return compact('comissao_usd','comissao_brl');
    }

    public function list(int $limit = 100, int $offset = 0, ?int $sellerId = null, ?string $ym = null, ?string $q = null): array
    {
        $where = [];$p = [];
        if ($sellerId) { $where[] = 'vn.vendedor_id = :sid'; $p[':sid'] = $sellerId; }
        if ($ym) { $where[] = "DATE_FORMAT(vn.data_lancamento, '%Y-%m') = :ym"; $p[':ym'] = $ym; }
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM vendas_nacionais WHERE id=:id');
        $stmt->execute([':id'=>$id]);
        $r = $stmt->fetch(PDO::FETCH_ASSOC);
        return $r ?: null;
    }

    public function create(array $data, int $sellerId): int
    {
        ...
        $stmt = $this->db->prepare('INSERT INTO vendas_nacionais (
            data_lancamento, numero_pedido, cliente_id, suite_cliente, peso_kg,
            valor_produto_usd, taxa_servico_usd, servico_compra_usd,
            frete_correios_brl, frete_correios_usd, produtos_compra_usd, taxa_dolar,
        ...
        $stmt->execute([
            ...
            ':frete_correios_brl' => $calc['frete_correios_brl'],
            ':frete_correios_usd' => $calc['frete_correios_usd'],
    ...
    public function update(int $id, array $data, string $sellerName, bool $allowDateChange): void
    {
        ...
        if (!$existing) return;
        if (!$allowDateChange) { $data['data_lancamento'] = $existing['data_lancamento']; }
        $calc = $this->compute($data);
        $ym = $this->monthOf($data['data_lancamento']);
        ...
        $stmt = $this->db->prepare('UPDATE vendas_nacionais SET
            ...
            valor_produto_usd=:valor_produto_usd, taxa_servico_usd=:taxa_servico_usd, servico_compra_usd=:servico_compra_usd,
            frete_correios_brl=:frete_correios_brl, frete_correios_usd=:frete_correios_usd, produtos_compra_usd=:produtos_compra_usd, taxa_dolar=:taxa_dolar,
